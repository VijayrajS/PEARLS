...
class UploadCSV(Resource):
    ...
    def post(self):
        ...
        if file and not UploadCSV.allowed_file(file.filename):
            ...
            response = Response(message, status=422, mimetype='application/json')
            response = enable_CORS(response, 'POST')
            return response
        
        file_path = os.path.join(app.config['UPLOAD_FOLDER'], return_hash(email), file.filename)
        file.save(file_path)
...
class ClusteringService(Resource):
    ...
    def post(self):
        ...
        dataset_object = Dataset(os.path.join(UPLOAD_FOLDER, return_hash(email), filename))
        ...
        filename = os.path.join(CLUSTER_JSON_FOLDER, return_hash(email), filename.split('.')[0]+'.json')
...
class RetrieveCluster(Resource):
    ...
    def post(self):
        ...
        cluster_json_path = os.path.join(CLUSTER_JSON_FOLDER, return_hash(email), filename)
...
class RetrieveDomains(Resource):
    ...
    def post(self):
        ...
        filename =  metadata['filename']
        file_path = os.path.join(app.config['UPLOAD_FOLDER'], return_hash(email), filename)
        
        domainJSON = returnDomainsJSON(file_path)
        response = Response(domainJSON, status=200, mimetype='application/json')
        response = enable_CORS(response, 'POST')
        return response
...
class RetrieveHeaders(Resource):
    ...
    def post(self):
        ...
        response = Response(json.dumps({'headers':header_list}), status=200, mimetype='application/json')
        response = enable_CORS(response, 'POST')
        return response
